  ...
  <div class="community">
    ...
    <div class="">
      <button class="ui primary big button" @click="openConversation">
        ...
        <span v-else
          >Ouvrir la conversation ({{ this.selectedUser.length }})</span
        >
      </button>
    </div>
  </div>
  ...
  methods: {
    ...
    openConversation() {
      ...
      if (this.selectedUser.length === 1) {
        ...
        let promise = this.createOneToOneConversation(this.selectedUser[0]);
        promise.finally(() => {});
      }
      if (this.selectedUser.length > 1) {
        let promise = this.createManyToManyConversation(this.selectedUser);
        promise.finally(() => {});
      }
      this.opening = false;
    },
